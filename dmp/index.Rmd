...
#   gitlab_url = "'https://gitlab.huma-num.fr/thuet/itineris",
#   private_token = Sys.getenv("GITLAB_COM_TOKEN"))
# url.root <- "https://raw.githubusercontent.com/zoometh/itineris/main/www/"
# url.root <- "https://gitlab.huma-num.fr/thuet/itineris/-/blob/main/www/"
url.root <- "C:/Rdev-itineris/itineris/www/img/"
htmltools::img(src = paste0(url.root, "logo-itineris.png"), 
               alt = 'logo', 
               width = '250px',
               style = 'position:absolute; top:0; right:0; padding:10px;')
# knitr::include_graphics(paste0(url.root, "logo.png"))
# print(getwd())
```
...
#local.dir <- "C:/Users/supernova/Dropbox/My PC (supernova-pc)/Documents/itineris_gl/"
# data.base <- 'https://gitlab.huma-num.fr/thuet/itineris/-/raw/main/data/'
...
root.idark <- "https://ark.frantiq.fr/ark:/"
...
df.info <- data.frame("Acronyme" = "Itineris",
                      ...
                      "Affiliation" = "AOROC UMR8546 CNRS-PSL",
...
Les données seront étudiées par des équipes françaises et italiennes. Par ailleurs, l'intéropérabilité des données requiert l'usage de l'anglais. Pour faciliciter les échanges, la table de correspondance multi-langues [lexicon.xlsx](https://gitlab.huma-num.fr/thuet/itineris/-/blob/main/data/lexicon.xlsx) est développée. Elle parmet d'aligner les termes, dont les IdArk. Ce fichier est lu à la volée par R:

```{r lexicon, echo = F, fig.cap = ""}
# TODO: changer local pour GitLab
# lexicon <- rio::import('https://github.com/zoometh/itineris/blob/main/fra-ita-eng/lexicon.xlsx?raw=true')
lexicon <- openxlsx::read.xlsx('C:/Rdev-itineris/itineris/data/lexicon.xlsx')
lexicon.idark.ok <- lexicon[!is.na(lexicon$IdArk), ]
# retrouve l'URL PACTOLS
lexicon.idark.ok$IdArk <- paste0("[", lexicon.idark.ok$IdArk, "]",
                                 "(", root.idark, lexicon.idark.ok$IdArk, ")")
lexicon.idark.ok.samp <- lexicon.idark.ok[sample(nrow(lexicon.idark.ok), 8), ]
# TODO: convert IdArk en URL
kable(lexicon.idark.ok,"html",
      row.names = F,
      caption = "Exemple des termes du thésaurus avec des IdArk renseignées") %>%
  kable_styling(full_width = FALSE,
                position = "center",
                font_size = 12)
```
...
example1.ITA <- rio::import('https://github.com/ANR-Itineris/itineris/blob/main/data/example_1.xlsx?raw=true')
```
...
```{r rdf, eval = FALSE, echo=TRUE}
base <- "https://github.com/ANR-Itineris/itineris/tree/main/"
rdf.label <- "NumLab"
rdf.type <- "Cu"
rdf.about <- "Source"
rdf <- rdf()
...
partners <- visNetwork::visNetwork(mbr_nodes.res, mbr_edges.res, 
  ...
  visNetwork::visEdges(shadow = TRUE,
                       ...
                       color = list(color = "lightblue", highlight = "red"))
partners
#htmlwidgets::saveWidget(partners, "C:/Rdev-itineris/itineris-anr/org/organisation.html")
```


## Work packages

### WP1: Identify the craft practices. Archaeometric analysis{#WP1}

*`This investigation promotes an extensive chemical & physical analysis performed in a complementary way by both French and Italian laboratories combining elemental, isotopic, metallographic and metallorganic analysis. This cross-methodology approach aims to: 
*  determine the alloy composition, the nature of raw materials and the manufacturing process:T.2A-C.
*  verify the possible existence of multiple active workshops:T.2 link to T.3
*  try finding sites of copper supply:T.2B
Pooling different but complementary analysis techniques will help to solve the current impasse in our understanding of the all stages of the bronze crafting process from the creation to the dissemination of finished artefacts(1)`*

### WP2: Social and cultural impact of the foreign productions and/or innovations in indigenous contexts. Archaeological & Technological analysis{#WP2}

*`This investigation takes into consideration all stages of the surface and internal structure investigation to provide a complete archaeological and technological screening. The morpho-stylistic features will be noted using specific nomad equipment and Digital analysis like 3D laser scanning, RTI & photogrammetry, Digital microscope imagery and X-radiography in addition to the traditional recording techniques. Moreover, the casting techniques and features will be tested by replication to verify the validity of inductive interpretations and to go far beyond the state of art in studying the different kind of techniques used or combined by ancient craftsmen(1)`*

### WP3: Proto-historic social networks: Data modelling & geo-statistic approach. Computational Archaeology{#WP3}

*`This work packageh includes two major aspects. The first goal is the development of a database dedicated computer tools to integrate, order, model, share and make reusable all the data previously collected, according to the different approaches. The second goal is to build a multilevel model of the metal handcraft organisation from the objects and/or craft techniques level, to the socio-economic networks level, in the three cultural areas investigated and their relationship with the Celtic communities. The aims is to be able to automatically order and spatially orient such complex and heterogeneous data and try to set out a dynamic typology of nested networks and sub-networks as well as different scenarios of the cultural and economic interactions(1)`*
...
Le thésaurus [PACTOLS](https://pactols.frantiq.fr/opentheso/index.xhtml) pour l'archéologie, depuis la préhistoire jusqu'à la période contemporaine et les sciences de l'Antiquité. Par exemple l'objet "fibule" de PACTOLS (IdArk: [26678/pcrt1tSmMf0Tw8](https://ark.frantiq.fr/ark:/26678/pcrt1tSmMf0Tw8)) est décrit par un graphe RDF enregistré dans la dossier [data/](https://gitlab.huma-num.fr/thuet/itineris/-/raw/main/data/TH_1_14737_.rdf). Nous le lisons en local avec le package `rdflib` et l'affichons sous forme de `jsonld`

```{r pactols, eval = FALSE, fig.height= 14, fig.width= 14, echo=TRUE, warning = F, fig.align='center', fig.cap=""}
library(rdflib)
options(rdf_print_format = "jsonld")
fibule.ark <- paste0(local.dir, "data/TH_1_14737_.rdf")
rdf <- rdf_parse(fibule.ark, format = "rdfxml") 
rdf
```
